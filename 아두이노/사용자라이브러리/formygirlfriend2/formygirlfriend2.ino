...
void getwheatherinfo(int year,int mon,int day,int hour){//날씨 정보를 획득합니다.
   ...
   if(mon<10){
    ...
    sprintf(httptext[0],"http://apis.data.go.kr/1360000/VilageFcstInfoService/getVilageFcst?serviceKey=MYkMEd1Kx5PWDad4K0CUzdminh1l8NocDD0sSSijoqivkQ%%2BdiwlZsSTq2pWRqO5KlmCFOlI5tA%%2B90TZhGDTaeA%%3D%%3D&numOfRows=80&base_date=%d%s%s&base_time=%s00&nx=60&ny=121&dataType=JSON&pageNo=1",year,s_mon,s_day,s_hour);
    Serial.println(httptext[0]);
    sprintf(httptext[1],"http://apis.data.go.kr/1360000/VilageFcstInfoService/getVilageFcst?serviceKey=MYkMEd1Kx5PWDad4K0CUzdminh1l8NocDD0sSSijoqivkQ%%2BdiwlZsSTq2pWRqO5KlmCFOlI5tA%%2B90TZhGDTaeA%%3D%%3D&numOfRows=80&base_date=%d%s%s&base_time=%s00&nx=60&ny=121&dataType=JSON&pageNo=2",year,s_mon,s_day,s_hour);
