  ...
  timeinfo=localtime(&now);
   ...
   if((timeinfo->tm_hour-3<5)){
    if((timeinfo->tm_hour-3<2)){
      getwheatherinfo(timeinfo->tm_year+1900,timeinfo->tm_mon+1,timeinfo->tm_mday-1,2,0);
      }else{
        getwheatherinfo(timeinfo->tm_year+1900,timeinfo->tm_mon+1,timeinfo->tm_mday,2,0);
        }
     getwheatherinfo(timeinfo->tm_year+1900,timeinfo->tm_mon+1,timeinfo->tm_mday-1,20,0);//0~2시사이니까 그전날 23시가 마지막
    }else {
         getwheatherinfo(timeinfo->tm_year+1900,timeinfo->tm_mon+1,timeinfo->tm_mday,2,0);
      if((timeinfo->tm_hour-3)%24%3==0)
              getwheatherinfo(timeinfo->tm_year+1900,timeinfo->tm_mon+1,timeinfo->tm_mday,(timeinfo->tm_hour-3+24)%24-1,0);
       if((timeinfo->tm_hour-3)%24%3==1)
               getwheatherinfo(timeinfo->tm_year+1900,timeinfo->tm_mon+1,timeinfo->tm_mday,(timeinfo->tm_hour-3+24)%24-2,0);
       if((timeinfo->tm_hour-3)%24%3==2)
               getwheatherinfo(timeinfo->tm_year+1900,timeinfo->tm_mon+1,timeinfo->tm_mday,(timeinfo->tm_hour-3+24)%24,0);
      }
}
void loop() {
//  getwheatherinfo();
  ...
  display.clearDisplay();
   ...
   if(((timeinfo->tm_hour==2&&timeinfo->tm_min==0)||(timeinfo->tm_hour==5&&timeinfo->tm_min==0)||(timeinfo->tm_hour==8&&timeinfo->tm_min==0)||(timeinfo->tm_hour==11&&timeinfo->tm_min==0)||(timeinfo->tm_hour==14&&timeinfo->tm_min==0)||(timeinfo->tm_hour==17&&timeinfo->tm_min==0)||(timeinfo->tm_hour==20&&timeinfo->tm_min==0)||(timeinfo->tm_hour==23&&timeinfo->tm_min==0))&&wloop==0){
         getwheatherinfo(timeinfo->tm_year+1900,timeinfo->tm_mon+1,timeinfo->tm_mday,(timeinfo->tm_hour-3+24)%24,1);  //한번 갱신하고 다시 실행 안되도록...
   ...
   display.clearDisplay();
     ...
     if(timeinfo->tm_hour%3==0)
       Serial.printf("%d %d %d %d\n",timeinfo->tm_hour,(timeinfo->tm_hour+3)%24,(timeinfo->tm_hour+6)%24,(timeinfo->tm_hour+9)%24);
     if(timeinfo->tm_hour%3==1)
       Serial.printf("%d %d %d %d\n",(timeinfo->tm_hour-1+24)%24,(timeinfo->tm_hour+2)%24,(timeinfo->tm_hour+5)%24,(timeinfo->tm_hour+8)%24);
     if(timeinfo->tm_hour%3==2)
       Serial.printf("%d %d %d %d\n",(timeinfo->tm_hour-2+24)%24,(timeinfo->tm_hour+1)%24,(timeinfo->tm_hour+4)%24,(timeinfo->tm_hour+7)%24);
...
void getwheatherinfo(int year,int mon,int day,int hour,int ini){//날씨 정보를 획득합니다.
    WiFiClient client;
   ...
   if(mon<10){
    ...
    sprintf(s_year,"http://apis.data.go.kr/1360000/VilageFcstInfoService/getVilageFcst?serviceKey=7503qUB6FNb%%2BuC5jrw1iycVdkNUroYZogV7Nav12eN5SdqjGsXoGzWv1OSJn4D7FTFHDfGgWas%%2FQVJbUF9T30g%%3D%%3D&numOfRows=50&base_date=%d%s%s&base_time=%s00&nx=57&ny=121&dataType=JSON",year,s_mon,s_day,s_hour);
...
for(int i=0;i<doc["response"]["body"]["numOfRows"].as<long>();i++){
     ...
     nt[0]=doc["response"]["body"]["items"]["item"][i]["fcstValue"].as<int>();
     break;
  }
    }
   ...
   for(int i=0;i<doc["response"]["body"]["numOfRows"].as<long>();i++){
     ...
     sky[0]=doc["response"]["body"]["items"]["item"][i]["fcstValue"].as<int>()+4; //sky와 중복을 피하기 위해
     break;
      }
  }
    } 
    if(sky[0]<5){
     ...
     sky[0]=doc["response"]["body"]["items"]["item"][i]["fcstValue"].as<int>();
     break;
  }
    } 
      }       
...
for(int i=0;i<doc["response"]["body"]["numOfRows"].as<long>();i++){
    ...
    for(int i=0;i<doc["response"]["body"]["numOfRows"].as<long>();i++){
     ...
     sky[1]=doc["response"]["body"]["items"]["item"][i]["fcstValue"].as<int>()+4; //sky와 중복을 피하기 위해
     break;
      }
  }
    } 
    ...
    for(int i=0;i<doc["response"]["body"]["numOfRows"].as<long>();i++){
     if(strcmp("SKY",doc["response"]["body"]["items"]["item"][i]["category"].as<char *>())==0){
     sky[1]=doc["response"]["body"]["items"]["item"][i]["fcstValue"].as<int>();
     break;
  }
    }     
  }
