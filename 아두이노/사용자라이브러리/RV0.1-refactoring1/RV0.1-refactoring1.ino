...
void getwheatherinfo(int year,int mon,int day,int hour){//날씨 정보를 획득합니다.
   ...
   if(mon<10){
    ...
    sprintf(httptext,"http://apis.data.go.kr/1360000/VilageFcstInfoService/getVilageFcst?serviceKey=MYkMEd1Kx5PWDad4K0CUzdminh1l8NocDD0sSSijoqivkQ%%2BdiwlZsSTq2pWRqO5KlmCFOlI5tA%%2B90TZhGDTaeA%%3D%%3D&numOfRows=80&base_date=%d%s%s&base_time=%s00&nx=57&ny=121&dataType=JSON",year,s_mon,s_day,s_hour);
...
void printNdaytoHday(int x,int y){ //숫자 날짜를 한글로 출력하는 함수
  ...
  if((stsv[i].fcstDate%100-timeinfo->tm_mday)==0){//오늘
     display.drawBitmap(x,y,todayfont,35,16, WHITE);
    }else if((stsv[i].fcstDate%100-timeinfo->tm_mday)==1){//내일
